# showcase-page-rendering Specification

## Purpose

Provide a single reusable `ShowcasePageLayout` that renders any `ShowcasePageDescriptor` into the established documentation scaffold — handling snippet loading, typed twin binding without reflection, escape hatches for divergent pages, and preservation of DocFX snippet markers on thinned pages.
...
When a documentation page is thinned to use the layout, the system SHALL retain its non-rendered
`@if (false)` snippet marker block verbatim so that existing `?name=` references from documentation
continue to resolve.
