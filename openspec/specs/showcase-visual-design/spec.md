# showcase-visual-design Specification

## Purpose

Establish a consistent, token-driven visual language for the showcase app built on Operon's Momentum design tokens and components, including dark-mode support, per-vendor accents, a live running-sequence timeline, and brand typography and motion that honors reduced-motion preferences.

## Requirements

### Requirement: Momentum-token visual language

The system SHALL ensure every showcase surface (shell, navigation, index, page layout, sub-tabs,
and the shared UI kit) expresses color, radius, and typography through Operon's Momentum design tokens
