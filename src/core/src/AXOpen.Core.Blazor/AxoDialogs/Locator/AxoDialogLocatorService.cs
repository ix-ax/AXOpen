...
public class AxoDialogLocatorService : IDisposable
{
    ...
    private readonly Dictionary<string, AxoDialogObserver> _observedDialogs = new();

    private readonly List<Guid> _subscribers = new();

    /// <summary>
    ///     Instantiates a new <see cref="AxoDialogLocatorService" />. Typically, this constructor is called only once.
    /// </summary>
    /// <param name="dialogLocatorPath">
    ///     The Path of the DialogLocator used for service identification in the dialogContainer
    ///     (typically the URL of the page where the dialogue is handled).
    /// </param>
    ...
    ///     The twin objects that may contain invokable dialogs from the controller to be handled by
    ///     this proxy service.
    /// </param>
    ...
    /// <summary>
    ///     Gets the locator path used for identifying the service in the dialog container, typically the URL of the page where
    ///     the dialogue is managed.
    /// </summary>
    ...
    internal void StopObservingDialogues(Guid dialogLocatorGuid)
    {
        if (_subscribers.Any(p => p == dialogLocatorGuid))
        {
            ...
            if (_subscribers.Count < 1) DisposeObservedDialogsHandling();
        }
    }
        ...
        if (senderAsDialogMonitor != null)
        {
            lock (_lockObject)
            {
                Log.Logger.Information(
                    $"AxoDialogLocatorService invoke dialog (from Plc): {senderAsDialogMonitor.Dialog.Symbol}");

                var exist = DisplayedDialogs.Any(p => p.Symbol == e.SymbolOfDialogInstance);
                if (!exist) DisplayedDialogs.Add(senderAsDialogMonitor.Dialog);
            }
    ...
    public void HandleDialogClosing_FromPlc(object? sender, AxoDialogEventArgs e)
    {
        ...
        if (senderAsDialogMonitor != null)
            ...
            {
                Log.Logger.Information(
                    $"AxoDialogLocatorService remove displayed dialog (from Plc): {senderAsDialogMonitor.Dialog.Symbol}");
