...
public partial class AxoDataFragmentExchange
{
    ...
    public void ExportData(string path, Dictionary<string, FragmentData> fragments = null, eExportMode exportMode = eExportMode.First, int firstNumber = 50, int secondNumber = 100, eFileType exportFileType = eFileType.csv, char separator = ';')
    {
        if (Path.GetExtension(path).Equals(".zip", StringComparison.OrdinalIgnoreCase))
        {
            ...
            foreach (var fragment in DataFragments)
            {
                fragment?.ExportData(Path.GetDirectoryName(path) + "\\exportDataPrepare\\" + fragment.ToString(), fragments, exportMode, firstNumber, secondNumber, exportFileType, separator);
            }
        ...
        else
        {
            foreach (var fragment in DataFragments)
            {
                fragment?.ExportData(Path.GetDirectoryName(path) + "\\" + fragment.ToString(), fragments, exportMode, firstNumber, secondNumber, exportFileType, separator);
            }
        }
    }
