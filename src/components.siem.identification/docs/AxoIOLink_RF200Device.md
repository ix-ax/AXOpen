...
_SIMATIC RF200 IO-Link RFID reader/writer_

Generated documentation for the `AxoIOLink_RF200Device` component.
...
View the library source at [`AxoIOLink_RF200Device.st`](https://github.com/Inxton/AXOpen/tree/3-unify-showcase/src/components.siem.identification/ctrl/src/AxoIOLink_RF200Device/AxoIOLink_RF200Device.st).
...
`AxoIOLink_RF200Device` does not ship a dedicated Blazor view. It renders via the generic `AxoComponent` pattern using `RenderableContentControl`, which inspects the component type at runtime and selects the matching rendering based on the `Presentation` attribute.
