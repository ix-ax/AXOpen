...
apax sld load --mode FULL --target $PLC_IP_ADDRESS --input $PLATFORM --username $USERNAME --password $PASSWORD --certificate $certfile --restart --accept-security-disclaimer 
if [[ $? -eq 0 ]]; then
